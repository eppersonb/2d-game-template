...
public class PlayerBehavior : MonoBehaviour
{
    ...
    */
    ...
    public IEnumerator startDashCoolDown() // cooldowntimer
    {
        dashAvailable = false;
    ...
    void wallSlide() // This function allows the player to wall slide.
    {
        ...
        if(isTouchingWall && !isGrounded && Input.GetKey(KeyCode.A)) //wallslide left
        {
            isWallSlinding = true;
            setWallJump(false);
        ...
        else if(isTouchingWall && !isGrounded && Input.GetKey(KeyCode.D))//wallslide right
        {
            isWallSlinding = true;
            setWallJump(false);
            myRigidBody2D.velocity = new Vector2(myRigidBody2D.velocity.x, Mathf.Clamp(myRigidBody2D.velocity.y, -wallSlideSpeed, float.MaxValue));
        ...
        {
            isWallSlinding = false; //currently not wallsliding.
        }
    }
